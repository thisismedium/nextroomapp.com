(function($){
  ...
  var checks = {
    first_name: function( val ){
      return (val == this.placeholder || /^\s*$/.test(val)) ? 'First Name is required.' : true;
    },
    last_name: function( val ){
      return (val == this.placeholder || /^\s*$/.test(val)) ? 'Last Name is required.' : true;
    },
  ...
  function validate(){
    ...
    if( id in checks )
      result = checks[ id ].call( this, this.value );
    else
      result = true;
